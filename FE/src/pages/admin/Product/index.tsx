...
const ListProducts = () => {
  ...
  const onFinish = (values: any) => {
    ...
    formDataRequest.append("description", values.description);
    formDataRequest.append("long_description", values.long_description);
    ...
    formData.variants.forEach((variant: any, index: any) => {
      formDataRequest.append(`variants[${index}][color]`, variant.color.id);
      formDataRequest.append(`variants[${index}][size]`, variant.size.id);
      formDataRequest.append(`variants[${index}][percent]`, variant.percent);
      formDataRequest.append(
        `variants[${index}][importPrice]`,
        variant.importPrice
      );
      formDataRequest.append(
    ...
    formData.images.forEach((image: any, index: any) => {
      ...
      formDataRequest.append(`images[${index}][file]`, image.image);
  ...
  // const disableVariation = !gia_nhap || !gia_ban;
              ...
              <>
                <div>
                  {selectedColors?.map((color: any) => (
                    <Button
                      key={color?.id}
                      // className={`${selectedColorForImages ? }!bg-black !text-white`}
                      className={
                ...
                {selectedColorForImages && (
                  <Upload
                    ...
                    //   .filter(
                    //     (item: any) =>
                    //       item.colorId == selectedColorForImages?.id
                    //   )
                    //   .map((item: any, idx: number) => ({
                    //     uid: `${selectedColorForImages?.id}-${idx}`,
