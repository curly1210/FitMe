...
const FormEditInforAccount = ({ refetchGetWallet, bankAccount }: any) => {
  ...
  const {
    mutate: mutateEditInforAccount,
    isPending: isloadingEditInforAccount,
  } = useCreate({
    resource: "wallet/update",
  });

  const onHandleEditInforAccount = (values: any) => {
    mutateEditInforAccount(
      { values },
      {
        onSuccess: (_response) => {
  ...
  const onFinish = (values: any) => {
    onHandleEditInforAccount(values);
    // console.log(values);
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-2xl  w-[400px]  transform transition-all duration-300 scale-100 animate-fade-in-up">
      ...
      >
        ...
        <div className="pt-4">
          <Button
            loading={isloadingEditInforAccount}
            htmlType="submit"
...
export default FormEditInforAccount;
